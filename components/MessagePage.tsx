        ...
        <div className="px-4 pt-4 pb-24 flex flex-col">
          {chats.map((chat, index) => (
            <div
              ...
              className={`flex items-center gap-3 bg-white/80 backdrop-blur-sm px-3 py-2 cursor-pointer active:bg-gray-100 transition-colors
                ${index === 0 ? 'rounded-t-xl border-b border-gray-100' : ''}
                ${index === chats.length - 1 ? 'rounded-b-xl' : ''}
                ${index > 0 && index < chats.length - 1 ? 'border-b border-gray-100' : ''}`}
            >
