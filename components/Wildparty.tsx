...
export default function Wildparty({ onClose }: WildpartyProps) {
  ...
  // Universal Viewport Auto-scaler
  ...
  useEffect(() => {
    const handleResize = () => {
      if (containerRef.current) {
        ...
        const parentH = containerRef.current.parentElement?.clientHeight || window.innerHeight * 0.7;
        const scaleX = parentW / 380;
        const scaleY = parentH / 540;
        setScaleFactor(Math.min(scaleX, scaleY, 1.15));
      }
    };
    handleResize();
  ...
  // IndexedDB Handler with 5 AM Purge
  ...
  // House-Edge Weighted Probability Pool (80% 5x animals to maximize house advantage)
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-end justify-center select-none overflow-hidden touch-none">
      ...
      {/* Universal Outer Frame */}
      <div
        ref={containerRef}
        className="relative bg-transparent w-full max-w-md rounded-t-3xl rounded-b-3xl shadow-2xl overflow-hidden flex items-center justify-center"
        style={{ height: '70vh', minHeight: '480px', maxHeight: '680px' }}
      ...
      >
        ...
        {/* Inner Coordinate Scaler */}
        <div
          className="relative w-[380px] h-[540px] flex-shrink-0 flex items-center justify-center transition-transform duration-75 origin-center"
          style={{ transform: `scale(${scaleFactor})` }}
        >
          {/* Top Header Bar */}
          {!loading && (
            <>
              {/* Top Left Icons (Sound & Rules/Help) */}
              <div className="absolute top-3.5 left-3.5 z-30 flex items-center gap-2">
                ...
                {/* Question Mark Button: Opens 40vh Rules Sheet */}
                <button
        ...
        {showHistorySheet && (
          <div className="absolute inset-x-0 bottom-0 h-[40vh] bg-[#0c0c0e]/95 backdrop-blur-xl border-t border-white/20 rounded-t-3xl shadow-[0_-10px_30px_rgba(0,0,0,0.8)] z-50 flex flex-col overflow-hidden animate-in slide-in-from-bottom duration-200">
            ...
            {/* 1st Row: Animals Header List (No Green Screen) */}
